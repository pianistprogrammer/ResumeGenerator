...
var RecorderController = function (element, service, recorderUtils, $scope, $timeout, $interval, PLAYBACK) {
  ...
  control.playbackRecording = function () {
    if (status.isPlaying || !service.isAvailable() || status.isRecording || !control.ngModel) {
      return false;
    }
  ...
  control.save = function (fileName) {
      alert('Came here');
    if (!service.isAvailable() || status.isRecording || !control.ngModel) {
      return false;
    }

    if (!fileName) {
      fileName = 'audio_recording_' + control.id + (control.ngModel.type.indexOf('mp3') > -1 ? 'mp3' : 'wav');
    }

    var blobUrl = window.URL.createObjectURL(control.ngModel);
    //var a = document.createElement('a');
   // var a = document.getElementById('audioUpload');
      var a = ("#audioUpload");
    a.href = blobUrl;
    a.target = '_blank';
    a.download = fileName;
