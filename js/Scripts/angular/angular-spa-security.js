...
.constant('security.urls', {
    ...
    registerApplicant: '/api/usermanagement/registerApplicant',
    login: '/token',
    ...
    userAuthInfo: '/api/usermanagement/getUserAuthInfo',
    resolvePermission: '/api/usermanagement/ResolvePermission',
    ...
    resolveApplicantPermission: 'api/application/resolveApplicantPermission',
    resolveUserRole: 'api/usermanagement/resolveUserRole',
    resolveAdminPermission: 'api/Service/resolveAdminPermission'
...
.factory('security.api', ['$http', 'security.urls', function ($http, Urls) {
    ...
    var Api = {
        ...
        getUserAuthInfo: function (email) {
            return $http({ url: Urls.userAuthInfo + '?email=' + email, method: 'GET' });
        },
        getResolvePermission: function (permission) {
            return $http({ url: Urls.resolvePermission + '?permission=' + permission, method: 'GET' });
        },
        login: function (data) {
        ...
        register: function (data) {
            return $http({ method: 'POST', url: Urls.register, data: data });
        },
        registerApplicant: function (data) {
            return $http({ method: 'POST', url: Urls.registerApplicant, data: data });
        },
        ...
        getResolveApplicantPermission: function (permission) {
            return $http({ url: Urls.resolveApplicantPermission + '?permission=' + permission, method: 'GET' });
        },
        getResolveUserRole: function (roles) {
            return $http({ url: Urls.resolveUserRole + '?roles=' + roles, method: 'GET' });
        },
        getResolveAdminPermission: function (permission) {
            return $http({ url: Urls.resolveAdminPermission + '?permission=' + permission, method: 'GET' });
        },
    };
...
.provider('security', ['security.urls', function (Urls) {
    ...
    securityProvider.createuserThenLogin = false;
    securityProvider.usePopups = true;
    ...
    securityProvider.$get = ['security.api', '$q', '$http', '$location', '$timeout', function (Api, $q, $http, $location, $timeout) {
        ...
        var resolvePermission = function () {
            var perms = 'canViewUserManagement,canViewWorkflow,canViewPaymentSetUp,canViewApproval,canViewReport,' +
                'canCreateSurveyPlan,canCreateBeacon,canViewPlanFiling,canViewCharts,canSearchPlanFiling,canViewAdminWorkflow,' +
          'canApply,canSetupState,canViewPayment,canViewAuditTrail,canMakePayment,canSetupPayment';


            Api.getResolvePermission(perms).success(function (resolved) {
        ...
        var resolveApplicantPermission = function() {
            var permission = 'canFillApplicationForm,canUpload,canFillParameters,canPrintForm';

            Api.getResolveApplicantPermission(permission).success(function (resolved) {
                Security.applicantPermissions = resolved;
            });

            Security.load = true;
        }

        var resolveUserRole = function() {
            var roles = 'StateAdmin,Applicant,SetupUser';

            Api.getResolveUserRole(roles).success(function(hasPermission) {
                Security.userRoles = hasPermission;
            });

            Security.load = true;
        }

        var resolveAdminPermission = function () {
            var permission = 'isBuildingOrAll,isBuilding';

            Api.getResolveAdminPermission(permission).success(function (resolved) {
                Security.adminPermissions = resolved;
            });

            Security.load = true;
        }

        var authInfo = function (user) {
            Api.getUserAuthInfo(user.UserName).success(function (userAuthInfo) {
                Security.userAuthInfo = userAuthInfo;
                Security.userrole = (userAuthInfo.State != undefined && userAuthInfo.State != '')
                    ? userAuthInfo.LastName + ' (' + userAuthInfo.Role + '/' + userAuthInfo.State + ')'
                    : userAuthInfo.LastName + ' (' + userAuthInfo.Role + ')';
            });
            resolvePermission();
            resolveAdminPermission();
            resolveApplicantPermission();
            resolveUserRole();
                ...
                if (accessToken() && associating()) {
                    ...
                    Api.getUserInfo(external_data.access_token).success(function (user) {
                        if (user.hasRegistered) {
                            ...
                            deferred.resolve(authInfo(user));
        ...
        Security.user = null;
        Security.userAuthInfo = null;
        Security.userrole = null;
        Security.externalUser = null;
        Security.token = accessToken();
        ...
        Security.registerApplicant = function (data) {
            var deferred = $q.defer();

            Api.registerApplicant(data).success(function () {
                if (securityProvider.events.registerApplicant) securityProvider.events.registerApplicant(Security); // Your Register events
                if (securityProvider.createuserThenLogin) {
                    Security.login(data).then(function (user) {
                        deferred.resolve(user);
                    }, function (errorData) {
